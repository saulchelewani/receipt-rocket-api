...
from core.auth import get_current_user, get_password_hash, has_permission, get_current_tenant_or_none
...
def create_user(user: UserCreate, db: Session = Depends(get_db), tenant: Tenant = Depends(get_current_tenant_or_none),
    ...
    if user.tenant_id and role.name == "global_admin":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Cannot create a global admin user for a tenant")
...
def create_db_user(user, db, tenant_id: UUID | None = None):
    ...
    db_user = User(email=str(user.email), role_id=user.role_id, hashed_password=get_password_hash(user.password),
                   tenant_id=tenant_id, name=user.name)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
